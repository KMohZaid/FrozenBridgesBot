...
def get_db_connection():
    ...
    with pool_lock:
        if pool is None:
            try:
                # Prefer DATABASE_URL if available (supports URL-encoded passwords)
                database_url = os.getenv("DATABASE_URL")
                if database_url:
                    pool = psycopg2.pool.SimpleConnectionPool(1, 20, dsn=database_url)
                    LOGGER.info("Database connection pool created successfully using DATABASE_URL.")
                else:
                    ...
                    LOGGER.info("Database connection pool created successfully using individual env vars.")
