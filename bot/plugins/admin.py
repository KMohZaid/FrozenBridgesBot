...
from ..__main__ import OWNER_ID, bot, running_games
from ..game import Player, GameState
from .game_management import get_lobby_keyboard
...
async def forcebridge_command(client: Client, message: Message):
    ...
    if not message.reply_to_message:
        return await message.reply_text(
            "You need to reply to a user's message to force add them to the game."
        )
    ...
    if target_player and target_player.is_active:
        return await message.reply_text(
            f"{target_user.mention} is already an active player in the game."
        )
    ...
    if target_player:
        ...
        await message.reply_text(
            f"🛡️ {role} {message.from_user.mention} forcibly reactivated {target_user.mention} in the game."
        )
    else:
        ...
        await message.reply_text(
            f"🛡️ {role} {message.from_user.mention} forcibly added {target_user.mention} to the game."
        )
    ...
    if game.game_state == GameState.WAITING:
        ...
        # Game is in progress - show player list reference
...
async def feedback_command(client: Client, message: Message):
    ...
    try:
        ...
        await message.reply_text(
            "✅ Feedback sent! Thank you for helping improve the bot!"
        )
...
async def commandlist_command(client: Client, message: Message):
    ...
    try:
        ...
        await message.reply_text(
            "Command list file not found. Please restart the bot.", quote=True
        )
    except Exception as e:
        await message.reply_text(
            f"An error occurred while reading the command list: {e}", quote=True
        )
