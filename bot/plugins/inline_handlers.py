...
async def inline_query_handler(client: Client, query: InlineQuery):
    ...
    if action == "dice" and query_text == os.getenv("DICE_INLINE_DISPLAY_KEY", "WAIFU_CUTE_DICE_ROLLER"):
        if (
            game.game_state == GameState.ROLLING
            and user_id in [game.current_player.user_id, game.answerer.user_id]
        ):
            results = [
                InlineQueryResultArticle(
                    id="roll",
                    title="Click to Roll!",
                    description="This will send a 🎲 to the chat.",
                    input_message_content=InputTextMessageContent("🎲"),
                )
            ]
            return await query.answer(results, cache_time=1, is_personal=True)
        else:
            return await query.answer([], cache_time=1, is_personal=True) # Not allowed to roll
