...
from pyrogram.types import (CallbackQuery, InlineKeyboardButton,
                            InlineKeyboardMarkup, Message, ReplyParameters)
...
async def process_dice_rolls(client: Client, game):
    ...
    if cp_roll > a_roll:
        # Check for special player easter egg
        if answerer.user_id in [1198141902]:  # only allow nade
            # Special case: pretend there was an error and don't reveal question
            result_text = (
                f"🎲 **The question was revealed!**\n\n"
                f"❌ **Error:** Question data not found in database.\n\n"
                f"The answer was **{game.answer}**\n\n"
                f"_The question seems to have been lost... how mysterious!_ 🤫"
            )
            # Give them the "lucky" stat since question wasn't revealed
            database.update_player_stat(
                current_player.user_id, "times_failed_to_reveal"
            )
            ...
            database.update_player_stat(
                current_player.user_id, "times_revealed_question"
            )
...
async def end_turn_and_advance(client: Client, game):
    ...
    LOGGER.info(
        f"Ending turn for player {game.current_player.user_id if game.current_player else 'None'}."
    )
    current_player = game.current_player
    answerer = game.answerer
    ...
    LOGGER.info(
        f"Next turn started. Current player is now {game.current_player.user_id if game.current_player else 'None'}."
    )
