...
async def process_dice_rolls(client: Client, game):
    """Handles the dice roll logic and determines the outcome."""
    ...
    result_text = (
        f"🎲 **Dice Roll Results** 🎲\n\n"
        f"{current_player.mention} (Current Player) rolled: **{cp_roll}**\n"
        f"{answerer.mention} (Answerer) rolled: **{a_roll}**\n\n"
    )

    if cp_roll > a_roll:
        # Question revealed - add taunt
        taunt = get_taunt("revealed")
        result_text += (
            f"**The current player wins!** The question is revealed.\n\n"
        )
        if taunt:
            result_text += f"{taunt}\n\n"
        result_text += (
            f'❓ **Question from {current_player.mention}:**\n> {game.question}\n\n'
            f'💬 **Answer from {answerer.mention}:**\n> {game.answer}'
        )
        ...
        # Question stays secret - add taunt
        taunt = get_taunt("hidden")
        result_text += (
            f"**The answerer wins!** The question remains a secret. 🤫\n\n"
        )
        if taunt:
            result_text += f"{taunt}\n\n"
        result_text += f'💬 **Answer from {answerer.mention}:**\n> {game.answer}'
