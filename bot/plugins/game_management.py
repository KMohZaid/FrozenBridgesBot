...
from .utils import send_turn_start_message, end_game_logic, is_user_in_any_active_game, send_player_list_with_ask_button
...
async def handle_lobby_callbacks(client: Client, query: CallbackQuery):
    ...
    if action == "lobby_join":
        ...
        # Game started successfully - edit lobby message to become player list
        game.player_list_message_id = game.lobby_message_id
...
async def leave_command(client: Client, message: Message):
    ...
    # Check if game should end (only if game has started)
    if game.game_state != GameState.WAITING and len(game.active_players) <= 1:
    ...
    if game.game_state == GameState.WAITING:
        ...
        # Game is in progress - update player list
...
**🗳️ Voting:**
/skipbridge - Vote to skip current turn (admins: skip any player)
/endbridge - Vote to end the game (admins: instant end)
/votekick @user - Vote to kick a player (admins: instant kick)
...
/skipbridge @user - Skip specific player (makes them inactive)
/votekick @user - Instantly kick a player from the game
...
• Fix turn tracking with player queue system
• Fix state clearing when players leave
• Prevent duplicate turns and stuck states

**Phase 2: Voting System Overhaul**
• Fix vote counting: (Players/2)+1 formula
• Show who voted in real-time
• Add `/votekick` command for permanent removal
• Add admin force commands (skip/kick)

**Phase 3: Timer System**
• Auto-skip if player doesn't ask (2min default)
• Auto-reject if no answer (3min default)
• Auto-roll dice if timeout (1min)
• Timer warnings (1min, 30sec, 10sec)
• Admin configurable via `/settimer`

**✨ New Features (Medium Priority)**

**Phase 4: Scoring & Statistics**
• 5-star difficulty rating system (1-5 points)
• `/mystats` - Personal statistics
• `/leaderboard` - Global rankings
• Enhanced end game summary

**Phase 5: Quality of Life**
• `/feedback` command for suggestions
• Optimize inline queries (more typing space)
• Fix dice animation on all platforms
• Better error messages

**Phase 6: Testing & Polish**
• Anti-abuse measures (spam protection)
• Performance testing
• Edge case handling
