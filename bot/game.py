...
class Game:
    ...
    def add_player(self, player: Player):
        """Adds a player to the game or reactivates them."""
    ...
    def remove_player(self, user_id: int):
        """Makes a player inactive and removes them from active queue."""
    ...
    def clear_turn_state(self):
        """Clears all turn-specific state."""
    ...
    def next_turn(self):
        """Sets the next active player as the current_player using queue-based approach."""
    ...
    def get_status_message(self) -> str:
        """Generates a clear and informative game status message."""
        if not self.players:
        ...
        inactive_players = [p for p in self.players if not p.is_active]
        ...
        if inactive_players:
            inactive_lines = [
                f" - {p.name} - {p.game_answer_count} points (Left)"
        ...
        # Add vote info if a vote is in progress (doesn't block gameplay)
        if self.vote_type:
            state_info += f"\n\n🗳️ **Vote in progress**: {self.vote_type}"
    ...
    def handle_player_leave(self, user_id: int):
        """Handles all state changes when a player leaves."""
    ...
    def validate_can_answer(self, user_id: int) -> tuple[bool, Optional[str]]:
        """Validates if a user can answer."""
